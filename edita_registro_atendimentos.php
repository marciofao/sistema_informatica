...
require_once 'logica_edita_registro.php';
...
<div class="container">	
		...
		<form action="" method="post">
			...
			<div class="cabecalho non-printable">
				...
				<div class="col-md-2 col-sm-2">
					...
					<input value="masculino" type="radio"  required="required" name="genero" aria-label="Genêro Masculino"
				...
				<div class="col-md-3 col-sm-3">
					...
					<select name="setor" id="setor" class="form-control">
						<option value="info" <?php if ($datas[0]['setor']=="info") echo "selected";?>>Informática</option>			
						<option value="om" <?php if ($datas[0]['setor']=="om") echo "selected";?>>Orientação e Mobilidade</option>
						<option value="psi" <?php if ($datas[0]['setor']=="psi") echo "selected";?>>Psicologia</option>
						<option value="avd" <?php if ($datas[0]['setor']=="avd") echo "selected";?>>Atividades da vida diária</option>
					</select>
				</div><!-- /.col-md-3 -->
						...
						<?php foreach ($avaliador as $av): 
							if ($av['nome']!='') { //se nome for vazio, não imprime este option
								...
								<option  value="<?php echo $av['cod'] ?>" 
									<?php 
								//IF else short-hand / ternario
									echo ''.($datas[0]['cod_avaliador']==$av['cod'] ? 'selected':''); 
									?>
									> <!--fecha option -->
									<?php echo $av['nome'] ?>
								</option>
	...
	<table class="printable table-responsive" >
		...
		<tr>
			...
			<td><?php echo $datas[0]['data_nasc']; ?></td>
	...
	<table class="table table-striped table-responsive">
		<thead>
			<tr>
				<th >Data</th>
				<th colspan="2">Atividade / Observação</th>
		...
		<tbody>
			...
			if ($atendimentos) {
						...
						foreach ($atendimentos as $data) { //ORDER ASC NAO FUNCIONA :'(
						...
						<tr>	
							...
							<td>
								...
								switch ($data['parecer']) {
									case '1':
									?> <span class="verd ">Concluído</span>  <?php
									break;
									case '2':
									?> <span class="amar " ;">Concluído com dificuldade</span>  <?php
									break;
									default:

									?> <span class="verm " ;">Não Concluído</span>  <?php
									break;
								}
								?>
							...
							<td >
								<button onclick="apaga(<?php echo $cod=$data['cod_atendimento'] ?>,<?php echo $cod=$data['cod_paciente'] ?>,'<?php echo date('d/m/y',strtotime($data['data'])) ?>')" type="button" class="close non-printable" aria-label="Excluir atividade">
				...
				<div class="non-printable">
					...
					<td>
						<label for="parecer" >
							<select name="parecer" id="parecer" class="form-control non-printable" style="width: 118px;" >
