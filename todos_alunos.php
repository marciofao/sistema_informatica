		...
		<div class="order">
			...
			<form method="post" onchange="submit()">
				<select aria-label="Selecionar ordem da lista" name="or">
					<option <?php echo ''.($or=='ultimo_atendimento' ? 'selected':''); ?>
					value="ultimo_atendimento">Último atendimento</option>
					<option <?php echo ''.($or=='nome' ? 'selected':''); ?>
					value="nome">Nome</option>
					<option <?php echo ''.($or=='data' ? 'selected':''); ?>
					value="data">Data de entrada</option>
				</select>
			</form>
		</div><!-- /.order -->
			...
			<tbody>
				<?php foreach ($datas as $data): ?>
					<tr>
						<td>
							<a  
							href="edita_registro_atendimentos.php?c=<?php echo $data['cod']; ?>
							<?php 
					...
					<td class="col-md-1">
						<a class="btn btn-danger" href="apagar_aluno.php?c=<?php echo $data['cod']; if ($_GET){ echo("&t=1"); }?>">Excluir</a>
					</td>
